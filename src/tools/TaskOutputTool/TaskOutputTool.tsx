	...
	if (task.task_type === "local_agent") {
		...
		if (result.retrieval_status === "success") {
			if (verbose) {
				...
				if ($[18] !== task.error) {
					t6 = task.error &&
						<Box flexDirection="column" marginTop={1}><Text color="error" bold={true}>Error:</Text><Box
							paddingLeft={2}><Text color="error">{task.error}</Text></Box></Box>;
					$[18] = task.error;
					$[19] = t6;
				} else {
					t6 = $[19];
			...
			if ($[27] !== expandShortcut) {
				t3 = <MessageResponse><Text dimColor={true}>Read output ({expandShortcut} to
					expand)</Text></MessageResponse>;
				$[27] = expandShortcut;
				$[28] = t3;
			} else {
				t3 = $[28];
