...
const LogoHeader = React.memo(function LogoHeader(t0: { agentDefinitions: AgentDefinitionsResult }) {
	...
	if ($[1] !== agentDefinitions) {
		t2 = <OffscreenFreeze><Box flexDirection="column" gap={1}>{t1}<React.Suspense fallback={null}><StatusNotices
			agentDefinitions={agentDefinitions}/></React.Suspense></Box></OffscreenFreeze>;
		$[1] = agentDefinitions;
		$[2] = t2;
	} else {
		t2 = $[2];
...
const MessagesImpl = ({
	...
	const lastThinkingBlockId = useMemo(() => {
		...
		for (let i = normalizedMessages.length - 1; i >= 0; i--) {
			...
			if (msg?.type === 'assistant') {
				...
				const hasToolResult = (msg.message.content as Array<{
					type: string
	...
	const renderMessageRow = (msg_8: RenderableMessage, index: number) => {
		...
		const row = <MessageRow key={k_0} message={msg_8} isUserContinuation={isUserContinuation}
		                        hasContentAfter={hasContentAfter} tools={tools} commands={commands}
		                        verbose={verbose || isItemExpanded(msg_8) || cursor?.expanded === true && index === selectedIdx}
		                        inProgressToolUseIDs={inProgressToolUseIDs} streamingToolUseIDs={streamingToolUseIDs}
		                        screen={screen} canAnimate={canAnimate} onOpenRateLimitOptions={onOpenRateLimitOptions}
		                        lastThinkingBlockId={lastThinkingBlockId} latestBashOutputUUID={latestBashOutputUUID}
		                        columns={columns} isLoading={isLoading} lookups={lookups_0}/>;
		...
		if (unseenDivider && index === dividerBeforeIndex) {
			return [<Box key="unseen-divider" marginTop={1}>
				<Divider title={`${unseenDivider.count} new ${plural(unseenDivider.count, 'message')}`} width={columns}
				         color="inactive"/>
			</Box>, wrapped];
		}
		return wrapped;
	...
	return <>
		...
		{isTranscriptMode && showAllInTranscript && hiddenMessageCount_0 > 0 &&
			...
			!disableRenderCap &&
			<Divider title={`${toggleShowAllShortcut} to hide ${chalk.bold(hiddenMessageCount_0)} previous messages`}
			         width={columns}/>}
		...
		{virtualScrollRuntimeGate ? <InVirtualListContext.Provider value={true}>
			<VirtualMessageList messages={renderableMessages} scrollRef={scrollRef} columns={columns}
			                    itemKey={messageKey} renderItem={renderMessageRow} onItemClick={onItemClick}
			                    isItemClickable={isItemClickable} isItemExpanded={isItemExpanded}
			                    trackStickyPrompt={trackStickyPrompt}
			                    selectedIndex={selectedIdx >= 0 ? selectedIdx : undefined} cursorNavRef={cursorNavRef}
			                    setCursor={setCursor} jumpRef={jumpRef} onSearchMatchesChange={onSearchMatchesChange}
