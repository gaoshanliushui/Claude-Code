...
export function TreeSelect(t0) {
	...
	if ($[34] !== focusNodeId || $[35] !== handleChange || $[36] !== handleFocus || $[37] !== hideIndexes || $[38] !== isDisabled || $[39] !== layout || $[40] !== onCancel || $[41] !== onUpFromFirstItem || $[42] !== options || $[43] !== visibleOptionCount) {
		t13 = <Select options={options} onChange={handleChange} onFocus={handleFocus} onCancel={onCancel}
		              defaultFocusValue={focusNodeId} visibleOptionCount={visibleOptionCount} layout={layout}
		              isDisabled={isDisabled} hideIndexes={hideIndexes} onUpFromFirstItem={onUpFromFirstItem}/>;
		$[34] = focusNodeId;
		$[35] = handleChange;
		$[36] = handleFocus;
		$[37] = hideIndexes;
