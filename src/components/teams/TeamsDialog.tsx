...
export function TeamsDialog({
	...
	if (dialogLevel.type === 'teammateList') {
		return <TeamDetailView teamName={dialogLevel.teamName} teammates={teammateStatuses}
		                       selectedIndex={selectedIndex} onCancel={onDone}/>;
	}
	if (dialogLevel.type === 'teammateDetail' && currentTeammate) {
...
function TeamDetailView(t0) {
	...
	if ($[0] !== selectedIndex || $[1] !== teammates) {
		t2 = teammates.length === 0 ? <Text dimColor={true}>No teammates</Text> :
			<Box flexDirection="column">{teammates.map((teammate, index) => <TeammateListItem key={teammate.agentId}
			                                                                                  teammate={teammate}
			                                                                                  isSelected={index === selectedIndex}/>)}</Box>;
		$[0] = selectedIndex;
		$[1] = teammates;
		$[2] = t2;
	} else {
	...
	if ($[3] !== onCancel || $[4] !== subtitle || $[5] !== t1 || $[6] !== t2) {
		t3 = <Dialog title={t1} subtitle={subtitle} onCancel={onCancel} color="background"
		             hideInputGuide={true}>{t2}</Dialog>;
		$[3] = onCancel;
		$[4] = subtitle;
		$[5] = t1;
		$[6] = t2;
	...
	if ($[8] !== cycleModeShortcut) {
		t4 =
			<Box marginLeft={1}><Text dimColor={true}>{figures.arrowUp}/{figures.arrowDown} select · Enter view · k kill
				· s shutdown · p prune
				idle{supportsHideShow && " \xB7 h hide/show \xB7 H hide/show all"}{" \xB7 "}{cycleModeShortcut} sync
				cycle modes for all · Esc close</Text></Box>;
		$[8] = cycleModeShortcut;
		$[9] = t4;
	} else {
		t4 = $[9];
...
function TeammateDetailView(t0) {
	...
	if ($[23] !== teammateTasks) {
		t9 = teammateTasks.length > 0 &&
			<Box flexDirection="column"><Text bold={true}>Tasks</Text>{teammateTasks.map(_temp2)}</Box>;
		$[23] = teammateTasks;
		$[24] = t9;
	} else {
		t9 = $[24];
	...
	if ($[25] !== promptExpanded || $[26] !== teammate.prompt) {
		t10 = teammate.prompt && <Box flexDirection="column"><Text
			bold={true}>Prompt</Text><Text>{promptExpanded ? teammate.prompt : truncateToWidth(teammate.prompt, 80)}{stringWidth(teammate.prompt) > 80 && !promptExpanded &&
			<Text dimColor={true}> (p to expand)</Text>}</Text></Box>;
		$[25] = promptExpanded;
		$[26] = teammate.prompt;
		$[27] = t10;
	} else {
	...
	if ($[28] !== onCancel || $[29] !== subtitle || $[30] !== t10 || $[31] !== t9 || $[32] !== title) {
		t11 = <Dialog title={title} subtitle={subtitle} onCancel={onCancel} color="background"
		              hideInputGuide={true}>{t9}{t10}</Dialog>;
		$[28] = onCancel;
		$[29] = subtitle;
		$[30] = t10;
		$[31] = t9;
	...
	if ($[34] !== cycleModeShortcut) {
		t12 = <Box marginLeft={1}><Text dimColor={true}>{figures.arrowLeft} back · Esc close · k kill · s
			shutdown{getCachedBackend()?.supportsHideShow && " \xB7 h hide/show"}{" \xB7 "}{cycleModeShortcut} cycle
			mode</Text></Box>;
...
function _temp2(task_0) {
	return <Text key={task_0.id}
	             color={task_0.status === "completed" ? "success" : undefined}>{task_0.status === "completed" ? figures.tick : "\u25FC"}{" "}{task_0.subject}</Text>;
}
