...
function BtwSideQuestion(t0) {
	...
	if ($[10] !== error || $[11] !== frame || $[12] !== response) {
		t7 = <ScrollBox ref={scrollRef} flexDirection="column" flexGrow={1}>{error ?
			<Text color="error">{error}</Text> : response ? <Markdown>{response}</Markdown> :
				<Box><SpinnerGlyph frame={frame} messageColor="warning"/><Text
					color="warning">Answering...</Text></Box>}</ScrollBox>;
		$[10] = error;
		$[11] = frame;
		$[12] = response;
		$[13] = t7;
	...
	if ($[17] !== error || $[18] !== response) {
		t9 = (response || error) &&
			<Box marginTop={1}><Text dimColor={true}>{UP_ARROW}/{DOWN_ARROW} to scroll · Space, Enter, or Escape to
				dismiss</Text></Box>;
		$[17] = error;
		$[18] = response;
		$[19] = t9;
	...
	if ($[20] !== handleKeyDown || $[21] !== t6 || $[22] !== t8 || $[23] !== t9) {
		t10 = <Box flexDirection="column" paddingLeft={2} marginTop={1} tabIndex={0} autoFocus={true}
		           onKeyDown={handleKeyDown}>{t6}{t8}{t9}</Box>;
		$[20] = handleKeyDown;
		$[21] = t6;
		$[22] = t8;
		$[23] = t9;
