...
## 七、`src/main.tsx` 中 `run()` 方法的拆分

### 7.1 拆分目标

`run()` 函数原本 3636 行（HEAD 1041-4678 行），由 6 个阶段组成：

1. 初始化 commander 程序（help 配置按长选项字母排序）
2. 注册 preAction hook（命令执行前的共享初始化）
3. 注册默认命令（`claude [prompt]`）的选项链与 action handler
4. 注册 feature flag 门控的额外顶层选项
5. 注册所有子命令（mcp / auth / plugin / skill / task / config / doctor / update / install / server 等 60+ 个）
6. 解析 argv，commander 派发到对应 action / 子命令

### 7.2 已完成的拆分

| 拆分 | 状态 | 说明 |
|------|------|------|
| `createSortedHelpConfig()` | ✅ | 行 1046；按长选项名字母序的 help 配置 |
| `installPreActionHook(program)` | ✅ | 行 1076；preAction 共享初始化 |
| `configureFeatureGatedOptions(program)` | ✅ | 行 1064；`.version()` 后的 feature flag 门控选项（含 `--worktree`、`--advisor`、ANT-only、KAIROS/HARD_FAIL 等） |

### 7.3 拆分经验与风险点

- **缩进一致性**：`run()` 内的代码块缩进为 1 tab。提取到顶层函数后仍保持 1 tab。**不要额外缩进或去缩进**——保持原始字符级原样。
- **Edit 而非 Python 脚本**：单次 Edit（基于精确字符串匹配）比 Python 行号切片更可靠——行号偏移会随前面的插入/删除累积。
- **行号 1-based vs 0-based**：Python `lines[a:b]` 是 `[a, b)` 区间，索引是 0-based；用 Edit 时行号是 1-based。转换时易出错。
- **保留空行**：提取后函数与函数之间留 1 个空行（与文件其余部分一致），函数结尾用 `\n` 而非 `\n\n`。

### 7.4 后续可继续的工作

- **阶段 3 拆分**：把 `program.name('claude')...action(...)...version(...)` 链（2831 行）抽到 `configureMainCommand(program)`。最大风险是 `.action()` 闭包内对 module-level 状态（`kairosGate`、`assistantModule`、外层 `feature()` 等）的引用——需要在不动闭包的前提下只切外壳。
- **阶段 5 拆分**：把 60+ 个 subcommand 注册（约 800 行）抽到 `registerAllSubcommands(program)`。风险在于 `.action()` 内 `await import('./cli/handlers/...')` 的 dynamic import 不能被打断。
- **阶段 3 内部细化**：action handler 闭包本身约 2806 行，包含 6 个并行初始化、continue/direct-connect/ssh/assistant/teleport 等多条 fast-path。可拆为 `prepareActionContext`、`parallelSetup`、`launchFreshRepl` 等子方法。

建议路径：用 Edit 逐步缩小 `run()`，每次只切一段、用 `git diff` 检查结构性正确性、`bun --check src/main.tsx` 验证语法、`bun run src/entrypoints/cli.tsx --help` 验证运行时正确性。
